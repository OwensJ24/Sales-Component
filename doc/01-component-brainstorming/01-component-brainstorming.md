...
- **Name**: <!-- TODO: fill with first and last name (e.g., Brutus Buckeye) and delete this comment --> Xzavier Owens
- **Dot Number**: <!-- TODO: fill with OSU dot number (e.g., buckeye.17) and delete this comment --> owens.1438
...
As a career I hope to be able to make applications as a software devloper. A personal hobby of mine is reselling on sites like ebay. Another hobby I have is working out, I also play a lot of sports like volleyball and basketball. I play a lot of games like overwatch and rocketleague to name a few. I enjoy running and have done marathons in the past before.
...
- Component Design #1: Ebay Inventory Management
  - **Description**: This component will make managing your sales and purchases on ebay much easier. It will take your sales on the ebay and sort and store them. It will also allow you to enter in and track your purchases when you make them. Using this information it will give you things like item profit, and helpful tax information.
  - **Kernel Methods**:
  Void addSale(): adds a sale to this
  Void  addPurchase(): adds a puchase to this
  Double calcProfit(): calculates profit of this


  - **Secondary Methods**:
  Double generateSalesReport(startDate, endDate): gives information on all sales from a certain time period
  Double generatePurchasesReport(startDate, endDate): gives information on all purchases from a time period
  Double calculateTax(year): calculates estimated taxes based on sales
  Void identifySlowMovingItems(threshold): sorts items based on length they have been listed for



<!-- TODO: brainstorm a component and delete this comment -->
- Component Design #2: Food tracker
  - **Description**: This tracks your food intake to ensure you meet your calorie intake to gain muscle. IT also helps track workouts as well.
  - **Kernel Methods**:
  void recordFoodIntake(): Records the calories in a meal
  void recordrun(runlength): Records a run and how long it was
  void recordBodyweight: Records current bodyweight
  - **Secondary Methods**:
  void goal(): sets a target body weight
  void setIntake(): Makes a daily calorie intake goal
  Double goalprogress(): shows how many pounds away from desired bodyweight
  Double dailyprogress(): shows how many calories away from daily progress
  void goalDaysleft():  calculates days left to reach bodyweight based on daily intake goal

<!-- TODO: brainstorm a component and delete this comment -->
- Component Design #3: Ski Trip Planner
  - **Description**: As an Skiier its important to pick the right day to go skiing. This component will get things like the upcoming forecast to let you know when the best day to ski is.
  - **Kernel Methods**:

    Double checkWeather(day): Retrieves weather forecast and return temperature for a certain day
    Boolean trailOpen(resort): Returns true if the specified resort is open
    String checkPrecipitation(day): Checks to see if it will be a clear day, snowing, or raining

  - **Secondary Methods**:
    int calculateBestDay(temp, clear, open): takes all the different variables and gives a score based on how good the day is
    int setBestDay(): sets how low a score can be for the day to be considered a "best day"
    int alertBestDay(): shows the user when a day comes into the forecast that is considered a "best day"
...
The component example given was very helpful, if anything more examples that consider different components would be even better.
